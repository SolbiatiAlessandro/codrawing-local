"""Run a local episode where every seat is a full Claude Code agent session.
    ...
    .venv/bin/python scripts/run_agent_episode.py --seats 4 --turns 10 --target "light bulb"

Each seat gets a persistent claude session (resumed across turns), its own
workspace directory under the run folder, and a per-turn trace file with its
full activity, token usage, and timing. Pass --policy-file to replace the
default policy prompt (the fixed game prompt stays the same). Artifacts land
in runs/agent-<slug>-<timestamp>/, including a self-contained report.html.
"""
...
async def run(
    ...
    try:
        ...
        for slot in range(seats):
            ...
            if policy_file:
                player_env["AGENT_POLICY_FILE"] = str(Path(policy_file).resolve())
...
def main() -> None:
    ...
    parser.add_argument("--policy-file", default=None, help="path to a policy prompt shared by all seats")
    args = parser.parse_args()
    asyncio.run(
        run(args.seats, args.turns, args.turns_per_round, args.target, args.model, args.port, args.policy_file)
    )
