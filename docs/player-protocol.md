...
When the config defines `teams`, two teams share the canvas and each team's
region is cropped and scored on its own. Observations then also carry:

```json
{
  "your_team": 0,
  "teams": [
    {"name": "Team A", "target": "pineapple",
     "region": {"x": 0, "y": 0, "width": 32, "height": 32}, "slots": [0, 1, 2, 3]},
    {"name": "Team B", "target": "strawberry",
     "region": {"x": 32, "y": 0, "width": 32, "height": 32}, "slots": [4, 5, 6, 7]}
  ],
  "team_feedback": [
    {"team": 0, "name": "Team A", "target": "pineapple", "target_score": 0.42, "score_delta": 0.01},
    {"team": 1, "name": "Team B", "target": "strawberry", "target_score": 0.37, "score_delta": -0.02}
  ]
}
```

A region is a scoring window, not a painting restriction: any player may
paint any pixel of the canvas, so an opponent's region can be drawn on or
erased. `image_model_feedback` repeats team 0's entry so single-target
viewers still work.

Board posts reach only the sender's team unless the post sets `"public":
true`:

```json
{"type": "message", "turn": 3, "text": "truce on row 0?", "public": true}
```

`recent_messages` is filtered to what that seat may see. The results file
records `final_scores`, `winner` (index, or `null` for a tie), `winner_name`,
and a per-team `score_trace`. The winner is decided by the score after the
FINAL turn; each team's best-ever score is recorded but wins nothing.
